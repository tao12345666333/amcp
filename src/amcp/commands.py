...
def _make_skills_command(skill_manager) -> SlashCommand:
    ...
    def skills_action(context: CommandContext, args: str) -> CommandResult:
        ...
        if subcommand == "list":
            ...
            if not skill_arg:
                return CommandResult(type="message", content="Please provide a skill name to activate.", message_type="error")
            if sm.activate_skill(skill_arg):
                return CommandResult(type="message", content=f"Skill '{skill_arg}' activated.", message_type="success")
            else:
                return CommandResult(type="message", content=f"Skill '{skill_arg}' not found or disabled.", message_type="error")
        
        elif subcommand == "deactivate":
            if not skill_arg:
                return CommandResult(type="message", content="Please provide a skill name to deactivate.", message_type="error")
            ...
            if not skill_arg:
                return CommandResult(type="message", content="Please provide a skill name to show.", message_type="error")
