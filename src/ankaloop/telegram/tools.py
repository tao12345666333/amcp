"""Telegram tools for AnkaLoop agents.
...
class TelegramSendTool(BaseTool):
    ...
    def __init__(self, bot_token: str | None = None) -> None:
        super().__init__()
        self._bot_token = bot_token or os.environ.get("ANKA_TELEGRAM_BOT_TOKEN", "")
    ...
    def execute(  # type: ignore[override]
        ...
        if not self._bot_token:
            return ToolResult(
                success=False,
                content="",
                error="Telegram bot token not configured. Set ANKA_TELEGRAM_BOT_TOKEN or pass bot_token.",
            )
...
Use this when a Telegram user asks AnkaLoop to proactively run a recurring task, for example
    ...
    def get_parameters_schema(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "notify": {
                    "type": "boolean",
                    "description": "Whether AnkaLoop should send non-silent results to the chat.",
                },
                "timeout": {
                    "type": "integer",
