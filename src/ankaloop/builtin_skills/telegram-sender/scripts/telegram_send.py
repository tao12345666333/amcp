...
"""
Telegram Bot Message Sender for AnkaLoop.
...
def main():
    ...
    send_parser.add_argument("--token", "-t", help="Bot token (defaults to ANKA_TELEGRAM_BOT_TOKEN env)")
    ...
    edit_parser.add_argument("--token", "-t", help="Bot token (defaults to ANKA_TELEGRAM_BOT_TOKEN env)")
    ...
    photo_parser.add_argument("--token", "-t", help="Bot token (defaults to ANKA_TELEGRAM_BOT_TOKEN env)")
    ...
    bot_token = getattr(args, "token", None) or os.environ.get("ANKA_TELEGRAM_BOT_TOKEN")
    if not bot_token:
        print("Error: Bot token required. Set ANKA_TELEGRAM_BOT_TOKEN env var or use --token")
        sys.exit(1)
